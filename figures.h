...
Le nombre ?afficher est soit un entier, soit un taux (de probabilit? entre  0 et 1 avec deux chiffres après la virgule.
...
On explique au début du figures.cpp comment est représent?un chiffre.
*/
...
protected:
	...
	Screen* _screen;
	int _x, _y;												//Ce sont les coordonnées du point du coin en haut ?gauche du nombre ?afficher.			
	...
	bool _rate;												//Indique si le nombre est un entier ou un taux (de probabilit?.
															//Un taux est un décimal entre 0 et 1 et doit donc être précéd?d'une virgule.
